...
import { CREATE_BUYER_API } from "@/lib/api";
...
const BuyerForm = ({ closeSheet }: Props) => {
	const form = useForm<Buyer>({
		resolver: zodResolver(CreateBuyerSchema),
		defaultValues: EmptyBuyerForm,
	});
	const router = useRouter();
	const submitForm = async (values: Buyer) => {
		try {
			const res = await post(CREATE_BUYER_API, values);
			if (res?.id) {
				toast("Success!", {
					description: "Buyer is created",
					action: { label: "Edit", onClick: () => router.push(`/buyer/${res.id}/edit`) },
				});
				return;
			}
			...
			<form id="buyer-form" onSubmit={form.handleSubmit(submitForm)} className="">
				<div className="create grid grid-cols-3 gap-4 py-4">
					<div className="col-span-3 flex flex-col md:col-span-1">
						<FormInput required label="Name" name="name" control={form.control} wrapperClassName="space-y-1" inputClassName="mt-1" />
					</div>
					<div className="col-span-3 flex flex-col md:col-span-1">
