...
import { patch, post } from "@/lib/api/fetch";
...
const InvoiceItemForm = ({ onSubmitted }: InvoiceItemFormProps) => {
	...
	const submitForm = async (values: InvoiceItem) => {
		const invoiceItem = { ...activeInvoiceItem, ...values };

		try {
			let res: { id: string } | null = null;
			const isEditInvoiceItem = !!invoiceItem.id;
			if (isEditInvoiceItem) {
				res = await patch(`${INVOICE_ITEM_API}/${invoiceItem.id}`, invoiceItem);
			} else res = await post(INVOICE_ITEM_API, invoiceItem);
			if (res?.id) {
				toast("Success!", {
					description: `Invoice item is ${isEditInvoiceItem ? "updated" : "created"}`,
				});
